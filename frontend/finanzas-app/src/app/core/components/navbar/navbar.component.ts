...
logout() {
    ...
    // 1. Limpiar AuthBridge si existe
    if (window.AuthBridge?.clearAuth) {
        //console.log('🔴 Limpiando AuthBridge...');
        window.AuthBridge.clearAuth();
    }
    
    // 2. Limpiar localStorage de Finanzas
    localStorage.removeItem('token');
    localStorage.removeItem('user');
    //console.log('🔴 LocalStorage limpiado');
    
    // 3. Eliminar cookies (usando la función que SÍ funciona)
    this.clearAllCookies();
    
    // 4. Redirigir a login de COMPRAS (no de Finanzas)
    //console.log('🔴 Redirigiendo a Compras login...');
    window.location.href = 'https://ryzenpc.mooo.com/#/login?force_logout=true&from=finanzas&t=' + Date.now();
